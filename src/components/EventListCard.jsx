    ...
    <div>
      ...
      <div className="bg-event-gradient text-white py-[24px] px-[14px] sm:px-[20px]">
        ...
        <p className="text-[11px] leading-[23.66px] line-clamp-2 font-[500]">
          {`${startDate.format("ddd, MMM DD YYYY, h:mm A")}  ${String(startDate.local()._d).split(" ")[5]} ${
            event.venue
          }`}
        </p>
        <h3 className="text-[11px] leading-[23.66px] font-medium line-clamp-1">Host: {event.host_name}</h3>
      </div>
    </div>
  );
}
