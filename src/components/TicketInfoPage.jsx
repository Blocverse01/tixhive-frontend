...
export default function TicketInfoPage() {
    ...
    useEffect(() => {
        if (isAuthenticated && isWeb3Enabled) {
            const eventFromMoralis = events.find((item) => item.contractAddress === contract);
            if (!eventFromMoralis) {
                setLoading(false);
                return;
            }
            ...
            <div className="page-wrapper">
                ...
                {isAuthenticated ?
                    <div>
                                ...
                                <div className={"text-white grid grid-cols-1 gap-5"}>
                                    <div className={"bg-event-gradient"}>
                                        <div
                                            ...
                                            <Link to={`/events/${contract}`} className={"py-1 bg-black-gradient px-2"}>
                                    ...
                                    <div className={"bg-event-gradient"}>
                                        ...
                                        <div>
                                            <div className={"grid-cols-3 text-center grid border-slate-300"}>
                                                ...
                                                <div className={"p-3 border-r"}>{ticketInfo.tokenId}</div>
                                                <div className={"p-3 border-r truncate"}>{ticketInfo.name}</div>
                                                <div className={"p-3"}>{ticketInfo.cost}</div>
                                                <div className={"p-3 col-span-3 border-t"}>Owner: {"  "}<span className={"underline text-brand-red"}><FontAwesomeIcon icon={solid("up-right-from-square")} /> {" "}<a target={"_blank"} rel={"noreferrer"} href={`https://polygonscan.com/address/${ticketInfo.owner}`}>{truncateEthAddress(ticketInfo.owner)}</a></span></div>
                                            </div>
                                        </div>
                                    </div>
                                </div> :
                        ...
                        {((ticketInfo.tokenId === undefined) && event) || ((ticketInfo.tokenId === undefined) && loaded)
