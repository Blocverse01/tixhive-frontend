  ...
  return (
    <section className="absolute inset-0 p-5 create-event-section">
      ...
      {processes.map((item, index) => (
        <div
          key={index}
          className={`mb-8 transition-all ease-in-out ${
            state >= index ? "" : "hidden"
          }`}
        >
          <div className="flex items-center">
            {state > index ? (
              <FontAwesomeIcon
                ...
                className="text-3xl text-green-400 md:text-5xl"
              />
            ) : state === index ? (
              <svg
                role="status"
                className="inline w-10 md:w-14 h-10 md:h-14 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-green-500"
                viewBox="0 0 100 101"
                fill="none"
